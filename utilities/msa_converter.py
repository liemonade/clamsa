...
def persist_as_tfrecord(dataset, out_dir, basename, splits=None, split_models=None, use_codons=False, use_compression=True, verbose=False):
    ...
    # Check whether splits is valid


    # Generate target file name based on chosen split and model
    target_path = lambda split_name, model: f"{out_dir}{basename}{'-'+split_name if split_name != None else ''}{'-m'+model if model != None else ''}.tfrecord{'.gz' if use_compression else ''}"
    ...
    with ExitStack() as stack:
        ...
        tfwriters = [[target_path(sn,m) for m in split_models] for sn in splits]
        print(tfwriters)
        return 
        ...
        for r, s in enumerate(ss):
            for j, p in enumerate(split_names):
                for model in models:
                    split_component = f"-{split_name}"
                    path = f"{out_dir}{basename}"
                    tfwriters[r][j][model] = \
