...
class MSA(object):
    def __init__(self, model = None, chromosome_id = None, start_index = None, end_index = None,
                 is_on_plus_strand = False, frame = 0, spec_ids = [], offsets = [], sequences = [], use_amino_acids = False, entry_length = 1):
    ...
    def codon_aligned_sequences(self, alphabet="acgt", gap_symbols='-'):
        ...
        c = 3 if self.entry_length == 1 else self.entry_length 
    ...
    def alilen(self, use_codons = False):
        ...
        if (self._coded_sequences is not None):
            ...
            if not use_codons and self.entry_length != 1:
                length = int(length / self.entry_length) 
        return length
...
def import_fasta_training_file(paths, undersample_neg_by_factor = 1., reference_clades = None, margin_width = 0, entry_length = 1, use_amino_acids = False):
    ...
    Args:
        ...
        entry_length (int): Length of an entry of the alphabet. e.g. 3 if you use codons or 1 if you use nucleotides as alphabet
...
def subsample_lengths(msas, use_codons, max_sequence_length = 14999, min_sequence_length = 1, relax = 1):
    ...
    for msa in msas:
        ...
        if use_codons:
            length = int(length / 3)
        if not use_codons and msa.entry_length != 1:
            length = int(length / msa.entry_length)
