...
class MSA(object):
    ...
    def coded_sequence(self, alphabet="acgt"):
        ...
        translated_alphabet = "".join(str(x) for x in range(len(alphabet))[::inv_coef])
        translation_table = str.maketrans(alphabet, translated_alphabet)

        # translate the list of sequences and convert it to a numpy matrix
        coded_sequence = np.array(
                [ list(S[::inv_coef].translate(translation_table)) for S in self.sequence ], 
                dtype=np.int)
    ...
    with gzip.open(path, 'rt') if path.endswith('.gz') else open(path, 'r') as f:
        ...
        for line in f:
            ...
            # parse the lines generated by GeneMSA::getMsa
                ...
                training_data[-1].sequence.append(seq)
